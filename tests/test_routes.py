...
def test_select_fund_page_with_tf_role(flask_test_client):
    ...
    assert '<a class="govuk-heading-m govuk-link--no-visited-state" href="/upload/TF/4" id="TF"> Towns Fund</a>' in str(
        page_html
    )
    assert (
...
def test_select_fund_page_with_pf_role(flask_test_client, mocked_pf_auth):
    ...
    page_html = BeautifulSoup(response.data)
    assert (
        '<a class="govuk-heading-m govuk-link--no-visited-state" href="/upload/TF/4" id="TF"> Towns Fund</a>'
        not in page_html
    )
    assert (
        '<a class="govuk-heading-m govuk-link--no-visited-state" href="/upload/PF/1" id="PF"> Pathfinders</a>'
